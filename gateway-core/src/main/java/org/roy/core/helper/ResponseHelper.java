...
import org.roy.core.context.IContext;
...
public class ResponseHelper {
	...
	public static FullHttpResponse getHttpResponse(ResponseCode responseCode) {
		...
		DefaultFullHttpResponse httpResponse = new DefaultFullHttpResponse(HttpVersion.HTTP_1_1, 
				...
				Unpooled.wrappedBuffer(gatewayResponse.getContext().getBytes()));
	...
	private static FullHttpResponse getHttpResponse(IContext ctx, GatewayResponse gatewayResponse) {
		...
		else if(gatewayResponse.getContext()!= null) {
			content = Unpooled.wrappedBuffer(gatewayResponse.getContext().getBytes());
		}
		else {
		...
		if(Objects.isNull(gatewayResponse.getFutureResponse())) {
			DefaultFullHttpResponse httpResponse = new DefaultFullHttpResponse(HttpVersion.HTTP_1_1, 
					gatewayResponse.getHttpResponseStatus(),
					 content);	
			httpResponse.headers().add(gatewayResponse.getResponseHeader());
			httpResponse.headers().add(gatewayResponse.getExtraResponseHeader());
			httpResponse.headers().set(HttpHeaderNames.CONTENT_LENGTH, httpResponse.content().readableBytes());
			return httpResponse;
		} else {
			gatewayResponse.getFutureResponse().getHeaders().add(gatewayResponse.getExtraResponseHeader());
